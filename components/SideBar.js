          ...
          >
            ...
            >
              ...
              <Template title={context.global.settings.language} center>
                <Toggle
                  isGhost={configs.lang !== "en"}
                  onClick={() => setConfigs({ ...configs, lang: "en" })}
                >
                  EN
                </Toggle>
                <Toggle
                  isGhost={configs.lang !== "zh-tw"}
                  onClick={() => setConfigs({ ...configs, lang: "zh-tw" })}
                >
                  繁
                </Toggle>
              </Template>
              <Template title={context.global.settings.weekStart.title} center>
                <Toggle
                  isGhost={configs.weekStartsOn !== 0}
                  onClick={() => setConfigs({ ...configs, weekStartsOn: 0 })}
                >
                  {context.global.settings.weekStart.sun}
                </Toggle>
                <Toggle
                  isGhost={configs.weekStartsOn !== 1}
                  onClick={() => setConfigs({ ...configs, weekStartsOn: 1 })}
                >
                  {" "}
                  {context.global.settings.weekStart.mon}
                </Toggle>
              </Template>
              <Template title={context.global.settings.timeFormat} center>
                <Toggle
                  isGhost={!configs.usePM}
                  onClick={() => setConfigs({ ...configs, usePM: true })}
                >
                  12H
                </Toggle>
                <Toggle
                  isGhost={configs.usePM}
                  onClick={() => setConfigs({ ...configs, usePM: false })}
                >
                  24H
                </Toggle>
              </Template>
              <Center w="full" mt="5">
                ...
                <a href="https://webcurate.co/p/meetor" target="_blank">
                  <img
                    ...
                    alt="Meetor Featured on WebCurate"
                    style={{ maxWidth: "250px", maxHeight: "54px" }}
                  />
                </a>
              </Center>
            </Popover.Body>
